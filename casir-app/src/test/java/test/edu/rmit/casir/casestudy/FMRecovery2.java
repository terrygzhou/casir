...
public class FMRecovery2 {
	static Logger logger = Logger.getLogger(FMRecovery2.class);
	...
	String compositeMRMPath = "./casestudy/epca/output/" + name + "_mrm.pm";
	...
	String rec2RawData = "./casestudy/epca/output/" + name + "_rec2_raw.txt";
	String rec2BigRawData = "./casestudy/epca/output/" + name + "_rec2_raw_big.txt";
	...
	public void setUp() throws Exception {
		...
		this.framework = new Framework(this.compositeDTMCPath,this.compositeMRMPath,null);
	...
	public void testLocalRecovery() {
		...
		Set<Integer> handlerLocCompletionStates = new HashSet<>();
		handlerLocCompletionStates.add(-1); // ERROR
		handlerLocCompletionStates.add(4); // supply_1
		handlerLocCompletionStates.add(5); // supply_0
		...
		startActions.add("avail_2");
//		startActions.add("na_2");

		global_rec1_com.virtualRun(10000, this.createSample(), this.createTimeCostMap(),
				this.createDollarCostMap(), this.rec2RawData, startActions, handlerLocCompletionStates,
				icStates);

//		global_rec1_com.virtualRun(100000, this.createSample(), this.createTimeCostMap(),
//				this.createDollarCostMap(), this.rec2BigRawData, startActions, handlerLocCompletionStates,
//				icStates);
	...
	private CompositePCA getTransformedModel() {
		...
		try {
			fspStr = FileHandler.readFileToSB(this.rec2Path).toString();
		...
		CompactState wdRec1 = comp.compile("WD_REC2", fspStr);
		APITest.printPCA(wdRec1);
		CompactState efm = comp.compile("EFM", efmFSPStr);
		APITest.printPCA(efm);

		CompactStateRef cs1 = new CompactStateRef(wdRec1);
		logger.info(cs1.getLTS());
		LabeledPCA lp1 = new LabeledPCA(wdRec1, null);
	...
	private Map<String, Double> createTimeCostMap() {
		...
		costMap.put("supply_0", 1.0);
		costMap.put("supply_1", 2.5);
	...
	private Map<String, Double> createDollarCostMap() {
		Map<String, Double> costMap = new HashMap<>();
		costMap.put("supply_0", 4.5);
		costMap.put("supply_1", 160.0);
