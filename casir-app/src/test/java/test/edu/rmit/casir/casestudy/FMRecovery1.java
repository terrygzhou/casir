...
public class FMRecovery1 {
	...
	String rec1Path = "./casestudy/epca/" + name + "_rec1.pca";
	String efmPath = "./casestudy/epca/" + name + "_efm.pca";
	String rec1RawData = "./casestudy/epca/output/" + name + "_rec1_raw.txt";
	String rec1RawBigData = "./casestudy/epca/output/" + name + "_rec1_raw_big.txt";
	...
	public void testLocalRecovery() {
		...
		handlerCompletionStates.add(4); // nowood
		handlerCompletionStates.add(5); // confirm_amount_1
		
		Set<Integer> icGloStates = new HashSet<Integer>();
		icGloStates.add(9);
		...
		global_rec1_com.virtualRun(10000, this.createSample(), this.createTimeCostMap(),
				this.createDollarCostMap(), this.rec1RawData, startActions, handlerCompletionStates,
				icGloStates);
		
		global_rec1_com.virtualRun(100000, this.createSample(), this.createTimeCostMap(),
				this.createDollarCostMap(), this.rec1RawBigData, startActions, handlerCompletionStates,
				icGloStates);
	}
	...
	private CompositePCA getTransformedModel() {
		...
		try {
			fspStr = FileHandler.readFileToSB(this.rec1Path).toString();
		...
		CompactState wdRec1 = comp.compile("WD_REC", fspStr);
...
//		logger.debug(lp1.getExtStates());
//		logger.debug(lp1.getLTS());
