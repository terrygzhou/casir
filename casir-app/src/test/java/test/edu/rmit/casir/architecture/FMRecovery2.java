...
public class FMRecovery2 {
	 ...
	 * output files below
	 */
	String compositeDTMCPath = "./casestudy/epca/output/" + name + "_rec2_dtmc.pm";
	String compositeMRMPath = "./casestudy/epca/output/" + name + "_rec2_";
	String composteDotPath = "./casestudy/epca/output/" + name + "_rec2.dot";
	...
	private void evaluate(Configuration afterConf, int bound) {
		...
		for (int i = 0; i < 99; i++) {
			String outputData = "./casestudy/epca/output/" + name + "_rec2_output_" + i + "_.txt";
	...
	private void setCostMap(Configuration conf) {
		...
		timeCost.put("supply_0", 1.0);
		timeCost.put("supply_1", 2.5);
		...
		dollarCost.put("supply_0", 4.5);
		dollarCost.put("supply_1", 160.);
	...
	private Vector<LabeledPCA> getCandidateLpca() {
		...
		try {
			transformedLpca = this.manualTransformLPCA(rec2Path, "WD_REC2",
