...
public class FMake2RecoveryMain {

	static Logger logger = Logger.getLogger(FMake2RecoveryMain.class);
	...
	public static void main(String[] args) throws Exception {
		...
		FMake2RecoveryMain fm = new FMake2RecoveryMain();
	...
	public void init() throws Exception {
		...
		// set costs
		// Map<String, Map<String, Double>> costMap = this.getCost(this.workDir +
		// "time_cost.txt",
		// "time");
		...
		Map<String, Map<String, Double>> costMap = this.getCost(this.workDir + "dollar_cost.txt",
				"dollar");
		this.costMap.putAll(costMap);
		conf.getCostMap().putAll(costMap);
	...
	public void recovery(int index, String outputSimDir, int gloState) throws InterruptedException {
		...
		try {
			...
			transformGloModel.outputMRMFile(
					this.recOutputDtmcDirVec.get(index - 1) + this.name + "_rec" + index, map);
