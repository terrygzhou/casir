...
public class FM {
	static Logger logger = Logger.getLogger(FM.class);
	...
	String ewdPath = "./casestudy/epca/" + name + ".pca";
	String efmPath = "./casestudy/epca/" + name + "_efm.pca";
	String fmRawData = "./casestudy/epca/output/" + name + "_raw.txt";
	String fmRawBigData = "./casestudy/epca/output/" + name + "_raw_big.txt";
	...
	public void testLocalRecovery() {
		...
		global_rec1_com.virtualRun(10000, this.createSample(), this.createTimeCostMap(),
				this.createDollarCostMap(), this.fmRawData, startActions, handlerCompletionStates,
				icGloStates);

		global_rec1_com.virtualRun(100000, this.createSample(), this.createTimeCostMap(),
				this.createDollarCostMap(), this.fmRawBigData, startActions,
				handlerCompletionStates, icGloStates);
	...
	private CompositePCA getTransformedModel() {
		...
		try {
			fspStr = FileHandler.readFileToSB(this.ewdPath).toString();
		...
		CompactState wdRec1 = comp.compile("EWD", fspStr);
