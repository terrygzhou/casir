 ...
 *
 */
public class FMRecovery4 {
	static Logger logger = Logger.getLogger(FMRecovery4.class);
	 ...
	 * output files below
	 */
	String compositeDTMCPath = "./casestudy/epca/output/" + name + "_rec4_dtmc.pm";
	String compositeMRMPath = "./casestudy/epca/output/" + name + "_rec4_";
	String composteDotPath = "./casestudy/epca/output/" + name + "_rec4.dot";
	...
	private void evaluate(Configuration afterConf, int bound) {
		...
		for (int i = 0; i < 99; i++) {
			String outputData = "./casestudy/epca/output/" + name + "_rec4_output_" + i + "_.txt";
		...
		completeActions.add("~request");
		completeActions.add("~login2");
		completeActions.add("wood_supply_1");
		completeActions.add("wood_supply_0");
	...
	private void setCostMap(Configuration conf) {
		...
		timeCost.put("wood_supply_0", 1.0);
		timeCost.put("wood_supply_1", 2.5);
		...
		dollarCost.put("wood_supply_0", 4.5);
		dollarCost.put("wood_supply_1", 160.);
	...
	private Vector<LabeledPCA> getCandidateLpca() {
		...
		try {
			transformedLpca = this.manualTransformLPCA(rec4Path, "WD_REC4",
