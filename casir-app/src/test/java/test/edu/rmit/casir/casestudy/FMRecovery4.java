...
public class FMRecovery4 {
	...
	String rec4RawData = "./casestudy/epca/output/" + name + "_rec4_raw.txt";
	String rec4RawBigData = "./casestudy/epca/output/" + name + "_rec4_raw_big.txt";
	...
	public void testLocalRecovery() {
		...
		Set<Integer> icGloStates = new HashSet<>();
		icGloStates.add(8);

		int recStartLocState = 1;
		...
		global_rec1_com.virtualRun(10000, this.createSample(), this.createTimeCostMap(),
				this.createDollarCostMap(), this.rec4RawData, startActions, handlerCompletionStates,
				icGloStates);
		
		global_rec1_com.virtualRun(100000, this.createSample(), this.createTimeCostMap(),
				this.createDollarCostMap(), this.rec4RawBigData, startActions, handlerCompletionStates,
				icGloStates);
	...
	private CompositePCA getTransformedModel() {
		...
		try {
			fspStr = FileHandler.readFileToSB(this.rec4Path).toString();
		...
		CompactState wdRec3 = comp.compile("WD_REC4", fspStr);
