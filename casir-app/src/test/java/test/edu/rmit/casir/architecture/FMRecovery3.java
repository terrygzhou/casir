...
public class FMRecovery3 {
	 ...
	 * output files below
	 */
	String compositeDTMCPath = "./casestudy/epca/output/" + name + "_rec3_dtmc.pm";
	String compositeMRMPath = "./casestudy/epca/output/" + name + "_rec3_";
	String composteDotPath = "./casestudy/epca/output/" + name + "_rec3.dot";
	...
	private void evaluate(Configuration afterConf, int bound) {
		...
		for (int i = 0; i < 99; i++) {
			String outputData = "./casestudy/epca/output/" + name + "_rec3_output_" + i + "_.txt";
	...
	private Vector<LabeledPCA> getCandidateLpca() {
		...
		try {
			transformedLpca = this.manualTransformLPCA(rec3Path, "WD_REC3",
